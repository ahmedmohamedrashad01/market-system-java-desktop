...
public class EditQty_purchases_normal_white_papersController implements Initializable {
    ...
    void btnSaveAction(ActionEvent event) {
        ...
        try {
            ...
            if (txtName.getText().isEmpty()) {
                ...
                a.setContentText("من فضلك أدخل اسم التاجر");
                a.show();
                ...
                PreparedStatement pst = d.con.prepareStatement("SELECT qty FROM purchase_fake_paper where id = '" + id + "'");
                ResultSet r = pst.executeQuery();
                if (r.next()) {
                    sqlQty = r.getDouble("qty");
                    if (sqlQty == qty) {
                       
                          String query = "UPDATE purchase_fake_paper SET name = '" + txtName.getText() + "',item = '"+item+"',size = '"+size+"',qty = '" + qty + "', mobile = '" + txtMobile.getText() + "' , date = '" + txtDate.getValue() + "' ,price = '" + price + "' where id = '" + id + "'";
                ...
                if (r.next()) {
                    ...
                    if (sqlQty == qty) {
                        ...
                        if (check > fullQTY) {
                            ...
                            new Alert(Alert.AlertType.ERROR, "الكميه المطلوبه غير متاحه").showAndWait();
    ...
    /*
       public void selctFullQTY() {
        try {
            DB d = new DB();
            ResultSet r2 = d.con.createStatement().executeQuery("SELECT qty FROM purchases_white_papers WHERE color = '" + color + "' and size = '" + size + "'");
            if (r2.next()) {
                fullQTY += r2.getDouble("qty");
            }
        } catch (SQLException ex) {
