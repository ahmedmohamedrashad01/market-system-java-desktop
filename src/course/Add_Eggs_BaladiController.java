...
public class Add_Eggs_BaladiController implements Initializable {
    ...
    void btnBackAction(ActionEvent event) {
         try {
            ...
            root4.getChildren().setAll(p);
        } catch (IOException ex) {
            Logger.getLogger(Add_EggsController.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    void btnSaveAction(ActionEvent event) {
         try {
            ...
            if (txtName.getText().isEmpty()) {
                ...
                DB d = new DB();
                d.check_Before_Insert(txtName.getText(), size,color, qty, txtMobile.getText(), txtDate.getValue(), price,user);
                txtName.clear();
                cbxBig.setSelected(false);
                cbxKilo300.setSelected(false);
                cbxSmall.setSelected(false);
