...
public class DepositController implements Initializable {
    ...
    @FXML
    private TableView<displayDeposit> myTable;

    @FXML
    private TableColumn<displayDeposit, Integer> colID;

    @FXML
    private TableColumn<displayDeposit, Double> colPrice;

    @FXML
    private TableColumn<displayDeposit, Date> colDate;
    ...
    void btnSearchAction(ActionEvent event) {
        ...
        if (fromDate.getValue() == null) {
            ...
            try {
                DB d = new DB();
                PreparedStatement pst = d.con.prepareStatement("select * from deposit where date between '" + fromDate.getValue() + "' and '" + toDate.getValue() + "'");
                ResultSet r = pst.executeQuery();
                while (r.next()) {
                    data.add(new displayDeposit(r.getInt("id"), r.getDouble("deposit_price"), r.getDate("date")));
    ...
    void btnSaveAction(ActionEvent event) {
        ...
        try {
            ...
            if (txtDeposit.getText().trim().isEmpty()) {
                ...
                DB d = new DB();
                PreparedStatement pst = d.con.prepareStatement("SELECT deposit_price FROM deposit where id = '" + id + "'");
                ResultSet r = pst.executeQuery();
                if (r.next()) {
                    sqlQty = r.getDouble("deposit_price");

                }
                if (sqlQty == inputPrice) {
                    String query = "UPDATE deposit SET deposit_price = '" + txtDeposit.getText().trim() + "', date = '" + dateDeposit.getValue() + "' where id = '" + id + "'";
    ...
    ObservableList<displayDeposit> data = FXCollections.observableArrayList();
    ...
    public void displayPriceOnly() {
        ...
        try {
            ...
            DB d = new DB();
            PreparedStatement pst = d.con.prepareStatement("SELECT SUM(deposit_price) AS Total FROM deposit");
            ResultSet r = pst.executeQuery();
            while (r.next()) {
    ...
    public void searchPrice() {
        ...
        try {
            ...
            DB d = new DB();
            PreparedStatement pst = d.con.prepareStatement("SELECT SUM(deposit_price) AS Total FROM deposit where date between '" + fromDate.getValue() + "' and '" + toDate.getValue() + "'");
            ResultSet r = pst.executeQuery();
            while (r.next()) {
                sum += r.getDouble("Total");
    ...
    public void displayData() {
        try {
            DB d = new DB();
            PreparedStatement pst = d.con.prepareStatement("select * from deposit");
            ResultSet r = pst.executeQuery();
            while (r.next()) {
                data.add(new displayDeposit(r.getInt("id"), r.getDouble("deposit_price"), r.getDate("date")));
    ...
    public class displayDeposit {
        ...
        public displayDeposit(int id, double price, Date date) {
            this.id = id;
            this.price = price;
            this.date = date;
        }
