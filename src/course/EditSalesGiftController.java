...
public class EditSalesGiftController implements Initializable {
    ...
    void btnSaveAction(ActionEvent event) {
        ...
        try {
            if (txtName.getText().isEmpty()) {
                ...
                DB d = new DB();
                PreparedStatement pst = d.con.prepareStatement("SELECT qty FROM sales_gift where id = '" + id + "'");
                ResultSet r = pst.executeQuery();
                if (r.next()) {
                    sqlQty = r.getDouble("qty");
                    if (sqlQty == qty) {
                        String query = "UPDATE sales_gift SET name = '" + txtName.getText() + "',item = '" + cbxItem.getValue() + "',qty = '" + qty + "' ,date = '" + txtDate.getValue() + "' , price = '" + price + "' where id = '" + id + "'";
